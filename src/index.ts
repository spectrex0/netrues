...
const jailRoles = new Map<string, string>();
let scamRules: { pattern: string }[] = [];
...
function aiBrain() {
  ...
  netrues.on("messageCreate", async (message) => {
    ...
    const isDuplicate = recent.length >= 2 &&
      recent.slice(0, -1).some(h =>
        content.trim().toLowerCase() === h.content.trim().toLowerCase()
      );
    ...
    if (content.length >= MIN_SCAM_LENGTH) {
      let isScamContent = scamRules.some((rule) =>
  ...
  for (const filePath of commandFiles) {
    try {
      ...
      const relativePath = filePath.replace(
        join(__dirname, "commands") + path.sep,
        ""
      );
...
async function startBot() {
  ...
  netrues.on("interactionCreate", async (interaction) => {
    ...
    if (!command) {
      console.error(
        `[SLASH] No command matching ${interaction.commandName} was found.`
      );
      return;
    }
    ...
    try {
      log(
        `[SLASH] Executing /${interaction.commandName} by ${
      ...
      console.error(
        `[SLASH] Error executing /${interaction.commandName}:`,
        error
      );
