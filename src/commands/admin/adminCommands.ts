...
export async function execute(interaction: any) {
  const helpEmbed = new EmbedBuilder()
    ...
    .addFields(
      { name: '`/jail`', value: 'Send a member to jail', inline: true },
      { name: '`/kick`', value: 'Kick a member from the server', inline: true },
      { name: '`/ban`', value: 'Ban a member from the server', inline: true },
      { name: '`/clear`', value: 'Delete up to 100 messages in this channel', inline: true },
      { name: '`/set-bot-presence`', value: 'Set Custom status', inline: true }
    )
